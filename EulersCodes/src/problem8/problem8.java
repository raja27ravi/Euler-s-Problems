package problem8;

import java.math.BigDecimal;
import java.math.BigInteger;
import java.util.ArrayList;
import java.util.List;

public class problem8 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		BigInteger myBigInteger = new BigInteger("");	
		BigInteger myBigDiv =new BigInteger("10");
		BigInteger myBigDiv1 =new BigInteger("10");
		BigInteger  dig =new BigInteger("0");
		
		long temp =1;
		
		int multifactor =13;
	
		List<Integer> al = new ArrayList<Integer>();
		
		while(myBigInteger.compareTo(BigInteger.ZERO) > 0) {
			
			dig = myBigInteger.mod(myBigDiv1);
			al.add(dig.intValue());
			myBigInteger = myBigInteger.divide(myBigDiv1);
			//System.out.println("Just to check how the things are going " + dig );
			myBigDiv = myBigDiv.multiply(myBigDiv1);
			//count++;
					
		}
	
		for(int i=0;i <= al.size() - multifactor;i++)
		{
			int count =0 ;
			long prod =1;
			for(int j=i;j<i+multifactor;j++)
			{
				prod = prod * al.get(j);
				count++;
				//System.out.println("j values is  :" + j);
				
			}
			System.out.println("prod value at " + i +" is :" + prod);
			if(i==0)
			{
				temp = prod;
			}
			if(prod>temp)
			{
				temp = prod;
			}
			System.out.println("i values is  :" + i);
		}
	
	System.out.println( "The largest product is :" + temp) ;
	
	}

	

}
